...
export class PanhellenicArchiveService {
  ...
  /**
   * Resolves a stored file name to an absolute path, refusing anything that would
   * escape the upload directory.
   */
  private resolveStoredFile(publicId: string): string {
    const safeName = path.basename(publicId);
    const resolved = path.resolve(ARCHIVE_UPLOAD_DIR, safeName);
    if (!resolved.startsWith(path.resolve(ARCHIVE_UPLOAD_DIR) + path.sep)) {
  ...
  /**
   * Removes a file multer already wrote to disk. Used when the request fails
   * after the upload landed, so we never leave a file without a database record.
   */
  private discardUploadedFile(file?: Express.Multer.File) {
    if (!file?.path) return;
    try {
      ...
      this.logger.warn(`Could not remove orphaned upload ${file.path}: ${error}`);
    }
  }
  ...
  async create(
    ...
    try {
      if (!isAllowedArchiveFile(file.mimetype, file.originalname)) {
        throw new BadRequestException(ARCHIVE_UNSUPPORTED_TYPE_MESSAGE);
      }

      if (file.size > ARCHIVE_MAX_FILE_SIZE) {
        throw new BadRequestException(ARCHIVE_TOO_LARGE_MESSAGE);
      }
      ...
      const originalFileName =
        file.originalname || `${createDto.displayName}.${file.mimetype.split('/')[1]}`;
  ...
  async delete(id: string, adminId: string) {
    ...
    if (file.publicId) {
      try {
        const filePath = this.resolveStoredFile(file.publicId);
        if (fs.existsSync(filePath)) {
          fs.unlinkSync(filePath);
        }
      } catch (error) {
        this.logger.error('Error deleting file from disk:', error);
