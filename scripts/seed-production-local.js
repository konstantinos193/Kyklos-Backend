...
async function seedProductionLocal() {
  // Use the Docker MongoDB URI from .env.docker
  const mongoUri = process.env.MONGODB_URI || 'mongodb://admin:KyklosMongo2024!Secure@localhost:27017/kyklos_db?authSource=admin';
...
seedProductionLocal().catch((error) => {
  console.error('❌ Failed to seed production local database:', error.message);
  process.exit(1);
});
